...
/*	var g = svg.append("g")
	...
	//		console.log(projection([coords[1],coords[0]]));
			selectedPoints.push(projection([coords[1],coords[0]]));
...
/*	d3.json('db.php', function (error, data) {
		...
		for(var i in data){
		  ...
		  selectedPoints.push([parseFloat(data[i].latitude), parseFloat(data[i]. longitude)]);
	...
	function clickMap () {
		selectedPoints.push(d3.mouse(this));
	//	selectedPoints.push(projection.invert(d3.mouse(this)));
		drawselectedPoints();
	}
	...
	function drawselectedPoints() {
		svg.selectAll('circle').data(selectedPoints).enter()
			.append('circle')
				.attr('cx', function (d) { return d[0]; })
				.attr('cy', function (d) { return d[1]; })
				.attr('r', 7)
	...
	function drawPaths(ipath) {
		var paths = _.map(_.zip(ipath.slice(0,ipath.length-1), ipath.slice(1)), function (pair) {
			return [selectedPoints[pair[0]], selectedPoints[pair[1]]]
		}).slice();
	...
	function run() {
	//	console.log('running', selectedPoints);

		var answer = tsp(selectedPoints, {});
	...
	function pathCost(path) {
		...
		return sum(_.map(zipped, function (pair) {
			return ccCost(selectedPoints[pair[0]], selectedPoints[pair[1]]);
		}));
	}
	function randomPath() {
	...
	function tsp(selectedPoints, opts) {
		opts = opts || {};
